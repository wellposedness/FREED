    ...
    def _phase_feed(self, inputs: list[dict], cycle_log: dict):
        ...
        for feed_idx, inp in enumerate(capped_inputs):
            ...
            prompt = (
                ...
                f"DETERMINISTIC GATE: a patch that adds a new function with zero callers "
                f"is auto-reverted. Only emit IMPLEMENT if you can name an existing "
                f"function or method that will invoke the new code in the SAME patch. "
                f"If the change is small enough to modify an existing function in place, "
                f"prefer that over adding a helper. Do NOT propose standalone scoring or "
                f"diagnostic helpers without a named caller — they will be reverted.\n"
